...
<div class="max-w-2xl mx-auto">
    ...
    <div class="mb-6 flex items-center gap-3">
        ...
        <div>
            <h1 class="text-xl font-bold text-gray-900">Edit Memory</h1>
            <p class="text-sm text-gray-500">ID #{{ $memory->id }} · Dibuat {{ $memory->created_at->diffForHumans() }}</p>
        </div>
    </div>

    {{-- Form --}}
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        <form action="{{ route('admin.memories.update', $memory) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')

            @include('admin.memories._form', ['mode' => 'edit', 'memory' => $memory])
